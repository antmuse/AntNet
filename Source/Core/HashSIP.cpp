/*
   ...
   3. Modify the prototype and implementation so that the function directly
      returns an u64 value, the hash itself, instead of receiving an
   ...
   6. Modified the original test.c file to be a stand-alone function testing
      the function in the new form (returing an u64) using just the
...
#define U8TO64_LE_NOCASE(p)                                                  \
    (((u64)(AppToLower((p)[0]))) |                                           \
     ((u64)(AppToLower((p)[1])) << 8) |                                      \
     ((u64)(AppToLower((p)[2])) << 16) |                                     \
     ((u64)(AppToLower((p)[3])) << 24) |                                     \
     ((u64)(AppToLower((p)[4])) << 32) |                                     \
     ((u64)(AppToLower((p)[5])) << 40) |                                     \
     ((u64)(AppToLower((p)[6])) << 48) |                                     \
     ((u64)(AppToLower((p)[7])) << 56))
...
u64 siphash(const void* inbuf, const size_t inlen, const u8* k) {
    const u8* in = reinterpret_cast<const u8*>(inbuf);
    ...
    switch(left) {
    case 7: b |= ((u64)in[6]) << 48; /* fall-thru */
    case 6: b |= ((u64)in[5]) << 40; /* fall-thru */
    case 5: b |= ((u64)in[4]) << 32; /* fall-thru */
    case 4: b |= ((u64)in[3]) << 24; /* fall-thru */
    case 3: b |= ((u64)in[2]) << 16; /* fall-thru */
    case 2: b |= ((u64)in[1]) << 8; /* fall-thru */
...
u64 siphash_nocase(const void* inbuf, const size_t inlen, const u8* k) {
    const u8* in = reinterpret_cast<const u8*>(inbuf);
#ifndef UNALIGNED_LE_CPU
    u64 hash;
    u8* out = (u8*)& hash;
    ...
    switch(left) {
    case 7: b |= ((u64)AppToLower(in[6])) << 48; /* fall-thru */
    case 6: b |= ((u64)AppToLower(in[5])) << 40; /* fall-thru */
    case 5: b |= ((u64)AppToLower(in[4])) << 32; /* fall-thru */
    case 4: b |= ((u64)AppToLower(in[3])) << 24; /* fall-thru */
    case 3: b |= ((u64)AppToLower(in[2])) << 16; /* fall-thru */
    case 2: b |= ((u64)AppToLower(in[1])) << 8; /* fall-thru */
    case 1: b |= ((u64)AppToLower(in[0])); break;
    case 0: break;
    }
...
//hash seed:sip
static u8 G_HASH_SEED_SIP[16];

void AppSetHashSeedSIP(const void* seed) {
    memcpy(G_HASH_SEED_SIP, seed, sizeof(G_HASH_SEED_SIP));
}

void* AppGetHashSeedSIP() {
    return G_HASH_SEED_SIP;
}

u64 AppHashSIP(const void* key, u64 len) {
    return siphash(key, len, G_HASH_SEED_SIP);
}

u64 AppCaseHashSIP(const void* buf, u64 len) {
    return siphash_nocase(buf, len, G_HASH_SEED_SIP);
}
...
/* --------------------------------- TEST ------------------------------------ */
#ifdef APP_SIPHASH_TEST

const u8 vectors_sip64[64][8] = {
...
s32 siphash_test(void) {
    ...
    u64 h1, h2;
    h1 = siphash((u8*)"hello world", 11, (u8*)"1234567812345678");
    h2 = siphash_nocase((u8*)"hello world", 11, (u8*)"1234567812345678");
    if(h1 != h2) fails++;
