...
namespace irr {
    namespace net {
        ...
        void AppPrintQueue(const s8* name, const CQueueRing* head){
            const CQueueRing *p;
            printf("<%s>: [", name);
            for (; !head->isEmpty(); ) {
                p = head->getNext(); 
        ...
        CQueueRing* CNetProtocal::createSegment(s32 size){
            return CQueueRing::createNode(size+sizeof(SKCPSegment), mMallocHook);
        }

        // delete a segment
        //void CNetProtocal::deleteSegment(SKCPSegment *seg){
        //	releaseMemory(seg);
        //}
        ...
        mMallocHook(0),
            ...
            mLogWriter(0){
                ...
                mQueueSend.init();
                mQueueReceive.init();
                mQueueSendBuffer.init();
                mQueueReceiveBuffer.init();
        }
        ...
        s32 CNetProtocal::receiveData(s8* iBuffer, s32 len){
            ...
            s32 fragment;
            len = 0;
            for (CQueueRing* p = mQueueReceive.getNext(); p!=&mQueueReceive; p=next) {
                ...
                if (ispeek == 0) {
                    p->delink();
                    CQueueRing::deleteNode(p, mFreeHook);
                    mReceiveQueueCount--;
                }
        ...
        s32 CNetProtocal::peekNextSize(){
            ...
            for (CQueueRing* p = mQueueReceive.getNext(); p != &mQueueReceive; p = p->getNext()) {
                seg = (SKCPSegment*) (p->getValue());
                length += seg->mLength;
                if (seg->mFrag == 0){
                    break;
            ...
            if (mStreamMode) {
                if (!mQueueSend.isEmpty()) {
                    ...
                    if (old->mLength < mMSS) {
                        ...
                        len -= extend;
                        node->delink();
                        CQueueRing::deleteNode(node, mFreeHook);
                    }
                }//if
                if (len <= 0) {
                    return 0;
        ...
        void CNetProtocal::updateACK(s32 rtt){
            ...
            rto = mSRTT + core::min_<u32>(mInterval, 4 * mRTT);
            mRTO = core::clamp<u32>(rto, mMinRTO, APP_NET_MAX_RTO);
        }


        void CNetProtocal::shrinkBuffer(){
            CQueueRing *p = mQueueSendBuffer.getNext();
        ...
        void CNetProtocal::parseACK(u32 sn){
            ...
            SKCPSegment* seg;
            CQueueRing* next;
            for (CQueueRing* p = mQueueSendBuffer.getNext(); p != &mQueueSendBuffer; p = next) {
                next = p->getNext();
                seg = (SKCPSegment*) (p->getValue());
                if (sn == seg->mSN) {
                    p->delink();
        ...
        // parse data
        //---------------------------------------------------------------------
        void CNetProtocal::parseSegment(CQueueRing* node){
        ...
        s32 CNetProtocal::import(const s8* data, long size){
            ...
            for (;;) {
                ...
                if (cmd == APP_NET_CMD_ACK) {
                    if (AppValueDiff(mTimeCurrent, ts) >= 0) {
                        updateACK(AppValueDiff(mTimeCurrent, ts));
                    }
                    parseACK(sn);
                    shrinkBuffer();
                    if (flag == 0) {
                        ...
                        if (AppValueDiff(sn, maxack) > 0) {
                            maxack = sn;
                        }
                    }
                    log(ENET_LOG_IN_DATA, 
                        "input ack: sn=%lu rtt=%ld rto=%ld", sn, 
                        (long)AppValueDiff(mTimeCurrent, ts),
                        (long)mRTO);
                } else if (cmd == APP_NET_CMD_PUSH_DATA) {
                    log(ENET_LOG_IN_DATA,  "input psh: sn=%lu ts=%lu", sn, ts);
                    if (AppValueDiff(sn, mReceiveNext + mMaxReceiveWindowSize) < 0) {
                        appendACK(sn, ts);
                        if (AppValueDiff(sn, mReceiveNext) >= 0) {
        ...
        void CNetProtocal::flush(){
            ...
            // flush data segments
            SKCPSegment *segment;
            for (CQueueRing* p = mQueueSendBuffer.getNext(); p != &mQueueSendBuffer; p = p->getNext()) {
        ...
        void CNetProtocal::update(u32 current){
            ...
            s32 slap = AppValueDiff(mTimeCurrent, mTimeFlush);
            ...
            if (slap >= 0) {
                mTimeFlush += mInterval;
                if (AppValueDiff(mTimeCurrent, mTimeFlush) >= 0) {
        ...
        u32 CNetProtocal::check(u32 current){
            if (mUpdated == 0) {
                return current;
            }
            u32 ts_flush = mTimeFlush;
