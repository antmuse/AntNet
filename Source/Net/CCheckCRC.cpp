...
*/
static const u16 AppCRC16Table[256] = {
...
*/
static const u32 AppCRC32Table[256] = {
...
static const u64 AppCRC64Table[256] = {
...
u16 CCheckCRC16::add(const void* buffer, u32 len) {
    if(buffer) {
        u16 ret = mResult;
        const u8* buf = (const u8*)buffer;
        while(len--) {
            ret = (ret << 8) ^ AppCRC16Table[(ret >> 8) ^ *buf++];
        }
        mResult = ret;
    }
    return mResult;
...
u32 CCheckCRC32::add(const void* buffer, u64 len) {
    u32 ret = mResult;
    if(buffer) {
        const u8* buf = (const u8*)buffer;
        while(len--) {
            ret = AppCRC32Table[(ret ^ *buf++) & 0xFF] ^ (ret >> 8);
        }
        mResult = ret;
    }
    return ret ^ 0xffffffff; //final
}

u64 CCheckCRC64::add(const void* buffer, u64 len) {
    u64 ret = mResult;
    if(buffer) {
        const u8* buf = (const u8*)buffer;
        while(len--) {
            ret = AppCRC64Table[(u8)ret ^ *buf++] ^ (ret >> 8);
        }
        mResult = ret;
    }
    return mResult;
...
int main(int argc, char** argv) {
    ...
    cc32 = c32.add("fc26498f", sizeof("fc26498f") - 1);  //cc32=1234567890
    cc64 = c64.add("fc26498f", sizeof("fc26498f") - 1);
    printf("c32=%u, c64=%llu\n", cc32, cc64);
    return 0;
}
#endif
