...
inline s32 isFileExtension(const core::CPath& filename,
    const core::CPath& ext0,
    const core::CPath& ext1,
    const core::CPath& ext2) {
...
//! search if a filename has a proper extension
inline bool hasFileExtension(const core::CPath& filename,
    const core::CPath& ext0,
    const core::CPath& ext1 = "",
    const core::CPath& ext2 = "") {
...
inline core::CPath& cutFilenameExtension(core::CPath &dest, const core::CPath &source) {
...
inline core::CPath& getFileNameExtension(core::CPath &dest, const core::CPath &source) {
    s32 endPos = source.findLast('.');
    if (endPos < 0)
        dest = "";
    else
...
inline core::CPath& deletePathFromFilename(core::CPath& filename) {
    // delete path from filename
...
//! trim paths
inline core::CPath& deletePathFromPath(core::CPath& filename, s32 pathCount) {
    // delete path from filename
    s32 i = filename.size();
...
inline s32 isInSameDirectory(const core::CPath& path, const core::CPath& file) {
    s32 subA = 0;
    s32 subB = 0;
    s32 pos;
...
static inline void splitFilename(const core::CPath &name, core::CPath* path = 0,
    core::CPath* filename = 0, core::CPath* extension = 0, bool make_lower = false) {
    s32 i = name.size();
    s32 extpos = i;
