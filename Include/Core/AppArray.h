...
public:
    ...
    //! Copy constructor
    TArray(const TArray<T, TAlloc>& other) : data(0) {
        *this = other;
    }
    ...
    TArray usage, but may use more memory than required by the data.
    */
    ...
    \param newStrategy New strategy to apply to this TArray. */
    ...
    //! Clears the TArray and deletes all allocated memory.
    void clear() {
    ...
    \param f If true, the TArray frees the allocated memory in its
    ...
    //! Gets last element.
    T& getLast() {
        APP_ASSERT(!used) // access violation
